...
## 4. 처음 실행할 때 나오는 파란 경고

> **Windows의 PC 보호 — Microsoft Defender SmartScreen에서 인식할 수 없는 앱의 시작을 차단했습니다.**

**정상입니다.** 코드 서명 인증서로 서명하지 않은 모든 프로그램에 뜨는 화면입니다.

**[추가 정보]** 를 누르고 → **[실행]** 을 누르면 실행됩니다.

경고를 없애려면 코드 서명 인증서(연 20~40만 원대, EV 인증서는 더 비쌈)를 사서
`electron-builder.yml` 의 `win:` 아래에 다음을 추가하면 됩니다.

```yaml
win:
  certificateFile: C:\경로\인증서.pfx
  certificatePassword: 비밀번호   # 실제로는 CSC_KEY_PASSWORD 환경 변수 사용을 권장
```
